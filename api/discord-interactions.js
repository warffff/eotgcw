...
function buildBookingEmbed(action, operation, planet, userId, sourceUrl){
  ...
  return {
    ...
    description: added
      ? `<@${userId}> забронировал проведение ${opText} у планеты **${clampText(planet, 120)}**. Ветка создаётся под этим сообщением; там будут указаны карты, аддоны и дата проведения.`
