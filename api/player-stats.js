...
function withModelImage(player, modelImages){
  if (!player) return player;
  if (!player.modelImage && player.model && modelImages && modelImages[player.model]) {
    player.modelImage = modelImages[player.model];
  }
  return player;
}
...
async function handleGet(req, res){
  ...
  const targetSteamId = steamId || linkedSteamId;
  if (!targetSteamId) {
    return sendJson(res, 200, { ok:true, player:null, linkedSteamId:'', total:Object.keys(stats.players || {}).length });
  }
  const player = withModelImage((stats.players || {})[targetSteamId] || null, modelImages);
