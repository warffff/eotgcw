...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === 'GET') {
      return sendJson(res, 200, { ok:true, steamId64:data.links[session.user.id] || '', discordId:session.user.id });
    }
    if (req.method === 'POST') {
      const body = await readBody(req);
      const steamId64 = cleanSteamId64(body.steamId64 || body.steamId);
