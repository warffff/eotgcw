...
const { GUILD_ID, canEditFromRoles, avatarUrl, getBaseUrl, setSessionCookie, setUserHintCookie } = require('../_utils');
module.exports = async (req, res) => {
  try {
    ...
    setUserHintCookie(res, sessionUser);
    res.statusCode = 302;
    res.setHeader('Cache-Control', 'no-store');
    res.setHeader('Location', '/?auth=success#home');
    res.end();
  } catch (err) {
    res.statusCode = 500;
