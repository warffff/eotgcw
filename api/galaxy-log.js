...
function resultPhrase(data){
  ...
  if (result === 'republic') return `${target} ${previous === 'republic' ? 'осталась' : 'перешла'} под контроль ВАР.`;
  ...
  if (event === 'fleet_launch') {
    return {
      ...
      fields: [
        ...
        { name:'Ветка', value:'Создаётся автоматически под этим сообщением для карт, аддонов и даты проведения.', inline:false }
      ]
    };
  }
...
function threadName(event, data){
  const prefix = event === 'diplomacy_launch' ? 'Дипломатия' : 'Боевой вылет';
  const planet = clampText(data.target || 'планета', 42);
  return `${prefix}: ${planet}`.slice(0, 95);
}
async function createEventThread(botToken, message, event, data){
  if (!message || !message.id || !['fleet_launch','diplomacy_launch'].includes(event)) return null;
  const threadRes = await fetch(`https://discord.com/api/v10/channels/${encodeURIComponent(CHANNEL_ID)}/messages/${encodeURIComponent(message.id)}/threads`, {
    method:'POST',
    headers:{ Authorization:`Bot ${botToken}`, 'Content-Type':'application/json' },
    body:JSON.stringify({
      name:threadName(event, data),
  ...
  if (thread && thread.id) {
    await fetch(`https://discord.com/api/v10/channels/${encodeURIComponent(thread.id)}/messages`, {
      method:'POST',
      headers:{ Authorization:`Bot ${botToken}`, 'Content-Type':'application/json' },
      body:JSON.stringify({
        content:'В этой ветке ивентолог указывает все необходимые карты и аддоны, а также дату проведения операции.'
      })
    }).catch(() => {});
  }
  return thread;
...
module.exports = async (req, res) => {
  ...
  if (!event) return sendJson(res, 400, { ok:false, error:'Missing event' });

  let fresh = { session:null, canEdit:false };
  try { fresh = await getFreshSession(req, res); } catch (_) {}

  if ((event === 'fleet_battle_result' || event === 'operation_final_result') && !fresh.canEdit) {
    return sendJson(res, 403, { ok:false, error:'Admin role required for this log event' });
  }

  const embed = buildEmbed(event, data, actorName(fresh.session));
  ...
  const thread = await createEventThread(botToken, message, event, data).catch(() => null);

  return sendJson(res, 200, { ok:true, message_id:message?.id || null, thread_id:thread?.id || null });
};
