...
const EDIT_ROLES = ['1509282764695011580'];
...
const STEAM_COOKIE_NAME = 'eotg_steam_session';
...
function readSteamSession(req){
  const session = readSignedSession(req, STEAM_COOKIE_NAME);
  if (!session || !/^\d{15,20}$/.test(String(session.steamId64 || ''))) return null;
  return session;
}
function setSteamSessionCookie(res, data){
  const secure = process.env.NODE_ENV === 'production' ? '; Secure' : '';
  const safe = {
    steamId64: String(data?.steamId64 || '').trim(),
    personaName: String(data?.personaName || '').trim(),
    avatar: String(data?.avatar || '').trim()
  };
  appendSetCookie(res, `${STEAM_COOKIE_NAME}=${encodeURIComponent(makeSession(safe))}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${SESSION_MAX_AGE_SECONDS}${secure}`);
}
function clearSteamSessionCookie(res){
  const secure = process.env.NODE_ENV === 'production' ? '; Secure' : '';
  appendSetCookie(res, `${STEAM_COOKIE_NAME}=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0${secure}`);
}
...
module.exports = { EDIT_ROLES, GUILD_ID, COOKIE_NAME, USER_HINT_COOKIE, STEAM_COOKIE_NAME, makeSession, readSession, readSteamSession, setSteamSessionCookie, clearSteamSessionCookie, setSessionCookie, setUserHintCookie, clearSessionCookie, canEditFromRoles, fetchDiscordMemberRoles, getFreshSession, avatarUrl, getBaseUrl, sendJson, truncate };
