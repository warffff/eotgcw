...
// v22: Player statistics from Garry's Mod server
...
function renderStatsPlayer(player){
  ...
  statsRpName.textContent = player.rpName || player.nickname || 'Без имени';
  ...
  if (player.modelGltf && statsModelViewer) {
    statsModelViewer.src = player.modelGltf;
    statsModelViewer.hidden = false;
    statsModelImage.hidden = true;
    statsModelImage.removeAttribute('src');
    statsModelFallback.hidden = true;
  } else if (player.modelImage) {
    if (statsModelViewer) { statsModelViewer.hidden = true; statsModelViewer.removeAttribute('src'); }
    statsModelImage.src = player.modelImage;
...
statsSteamLogout?.addEventListener('click', async () => { try { await fetch('/api/steam/logout', {method:'POST'}); } catch(_){} localStorage.removeItem('eotg_stats_steamid64'); await loadPlayerStats(true); });
