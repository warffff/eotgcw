...
(function(){
  ...
  function finishBattleEarlyRandom(){
    ...
    if(!confirm('Закончить битву флота досрочно? Победитель будет определён случайно.')) return;
    ...
    s.log=[result==='republic'
      ? `Администратор досрочно завершил битву флота. Рандомный исход: победили силы ВАР. Планета ${s.target} перешла под контроль Республики.`
      : `Администратор досрочно завершил битву флота. Рандомный исход: победили силы КНС. Планета ${s.target} осталась под контролем КНС.`];
    save(s);
    applyControls();
    stopMusic();
    renderGalaxy();
  ...
  function statusHtml(s){
    ...
    if(s.status==='awaiting_admin'){
      ...
      return `<p>Бой завершён. Планета пока остаётся во вражеском статусе, а Venator удерживают орбиту до решения администратора.</p>${hpHtml(s)}${admin ? adminHtml(s) : '<p class="galaxy-war-warning">Ожидается администратор с Discord-ролью для завершения боевого вылета.</p>'}`;
    }
