...
(function(){
  ...
  function battlePositions(target, now, status){
    ...
    const sway=Math.sin(now/900)*7;
    return {
      rep:[0,1,2].map(i=>({x:target.x-78+sway+Math.sin(now/700+i)*5,y:target.y-36+i*36+Math.cos(now/800+i)*4,rot:0})),
      cis:[0,1,2].map(i=>({x:target.x+78-sway+Math.sin(now/740+i)*5,y:target.y-36+i*36+Math.cos(now/880+i)*4,rot:180}))
    };
  }
  function addLaser(ns, layer, from, to, cls, phase){
    const l=document.createElementNS(ns,'line');
    const t=(Math.sin(phase)+1)/2;
    const x1=from.x+(to.x-from.x)*t*.34;
    const y1=from.y+(to.y-from.y)*t*.34;
    const x2=from.x+(to.x-from.x)*(t*.34+.38);
    const y2=from.y+(to.y-from.y)*(t*.34+.38);
    l.setAttribute('x1',x1.toFixed(1)); l.setAttribute('y1',y1.toFixed(1));
    l.setAttribute('x2',x2.toFixed(1)); l.setAttribute('y2',y2.toFixed(1));
    l.setAttribute('class',`galaxy-laser ${cls}`);
    layer.appendChild(l);
    if(cls==='red'){
      const c=document.createElementNS(ns,'circle');
      c.setAttribute('cx',x2.toFixed(1)); c.setAttribute('cy',y2.toFixed(1)); c.setAttribute('r','2.6');
      c.setAttribute('class','galaxy-plasma-bolt red');
      layer.appendChild(c);
    }
  }
  function renderShips(){
    ...
    if(s.status==='battle'){
      ...
      const fade=Math.min(1,Math.max(.08,(now-(s.battleAt||now))/2600));
      const pos=battlePositions(target, now, s.status);
      pos.rep.forEach(p=>layer.appendChild(img(ns,assets.venator,p.x,p.y,40,23,'galaxy-ship republic-ship battle',p.rot,fade)));
      pos.cis.forEach((p,i)=>{
        const startX=target.x+140+i*18;
        const startY=target.y-62+i*42;
        const x=startX+(p.x-startX)*fade, y=startY+(p.y-startY)*fade;
        layer.appendChild(img(ns,assets.cis,x,y,34,20,'galaxy-ship cis-ship battle entering',p.rot,fade));
      });
      if(fade>.45){
        for(let i=0;i<9;i++) addLaser(ns,layer,pos.rep[i%3],pos.cis[(i+1)%3],'blue',now/130+i*1.7);
        for(let i=0;i<7;i++) addLaser(ns,layer,pos.cis[i%3],pos.rep[(i+2)%3],'red',now/150+i*1.9);
      }
      return;
    }
    if(s.status==='awaiting_admin'){
