...
const statsSteamInput = document.getElementById('statsSteamInput');
const statsSaveLink = document.getElementById('statsSaveLink');
...
function renderStatsPlayer(player){
  ...
  if (!player) {
    ...
    statsModelPath.textContent = 'Данные от сервера ещё не получены';
    statsModelImage.hidden = true;
    statsModelImage.removeAttribute('src');
    statsModelFallback.hidden = false;
    return;
  }
  ...
  statsModelPath.textContent = player.model || 'Модель не указана';
  if (player.modelImage) {
...
async function loadPlayerLink(){
  if (!authState.user) return localStorage.getItem('eotg_stats_steamid64') || '';
  try {
  ...
  return localStorage.getItem('eotg_stats_steamid64') || '';
}
async function loadPlayerStats(force = false){
  ...
  if (statsSteamInput && linked) statsSteamInput.value = linked;
  if (!linked) {
    statsLoadedOnce = true;
    renderStatsPlayer(null);
    setStatsStatus('Введите SteamID64 и нажмите «Привязать».', 'warn');
    return;
  }
  try {
    ...
    else setStatsStatus('SteamID привязан, но Garry\'s Mod сервер ещё не отправил данные по этому игроку.', 'warn');
  } catch (err) {
    setStatsStatus('Не удалось загрузить статистику: ' + err.message, 'err');
  }
}
async function savePlayerLink(){
  const steamId = String(statsSteamInput?.value || '').trim();
  if (!isSteamId64(steamId)) {
    setStatsStatus('Укажите корректный SteamID64.', 'err');
    return;
  }
  localStorage.setItem('eotg_stats_steamid64', steamId);
  if (!authState.user) {
    setStatsStatus('SteamID сохранён в браузере. Для привязки к Discord авторизуйтесь.', 'warn');
    await loadPlayerStats(true);
    return;
  }
  try {
    statsSaveLink.disabled = true;
    setStatsStatus('Сохраняю привязку SteamID к Discord...');
    await apiJson('/api/player-link', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({ steamId64:steamId })
    });
    setStatsStatus('SteamID привязан к Discord-аккаунту.', 'ok');
    await loadPlayerStats(true);
  } catch (err) {
    setStatsStatus('Не удалось сохранить привязку на сайте: ' + err.message + '. SteamID сохранён локально в браузере.', 'warn');
    await loadPlayerStats(true);
  } finally {
    statsSaveLink.disabled = false;
  }
}
statsSaveLink?.addEventListener('click', savePlayerLink);
statsRefresh?.addEventListener('click', () => loadPlayerStats(true));
statsSteamInput?.addEventListener('keydown', (event) => { if (event.key === 'Enter') savePlayerLink(); });
...
loadEditableContent().then(loadAuthState).then(() => loadPlayerStats(false));
