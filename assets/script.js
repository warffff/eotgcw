...
(function(){
  ...
  function addShotSprite(ns, layer, from, to, href, cls, phase, width, height){
    if(!from || !to) return;
    const dx=to.x-from.x, dy=to.y-from.y;
    const angle=Math.atan2(dy,dx)*57.2958;
    const flight=0.14 + (((Math.sin(phase)+1)/2) * 0.72);
    const x=from.x + dx*flight;
    const y=from.y + dy*flight;
    const shot=img(ns, href, x, y, width, height, `galaxy-shot ${cls}`, angle, .98);
    layer.appendChild(shot);
    const impact=document.createElementNS(ns,'circle');
    const lead=Math.min(0.98, flight + 0.05);
    impact.setAttribute('cx',(from.x + dx*lead).toFixed(1));
    impact.setAttribute('cy',(from.y + dy*lead).toFixed(1));
    impact.setAttribute('r', cls==='red' ? '2.4' : '1.9');
    impact.setAttribute('class', `galaxy-shot-impact ${cls}`);
    impact.setAttribute('opacity', String(0.35 + (((Math.sin(phase*1.7)+1)/2)*0.45)));
  ...
  function renderShips(){
    ...
    if(s.status==='battle'){
      ...
      pos.cis.forEach((p,i)=>{
        ...
        layer.appendChild(img(ns,assets.cis,x,y,44,26,'galaxy-ship cis-ship battle entering',p.rot,Math.max(.12,fade)));
      });
      if(fade>.34){
        for(let i=0;i<10;i++) addShotSprite(ns,layer,pos.rep[i%3],pos.cis[(i+1)%3],assets.laserBlue,'blue',now/155+i*1.35,34,10);
        for(let i=0;i<8;i++) addShotSprite(ns,layer,pos.cis[i%3],pos.rep[(i+2)%3],assets.laserRed,'red',now/140+i*1.6,42,15);
      }
      return;
    }
    if(s.status==='awaiting_admin'){
