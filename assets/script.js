...
(function(){
  const storeKey = 'eotgGalaxyWarV2';
  ...
  function state(){ const s=read(storeKey, null); return s&&s.target ? update(s) : null; }
  function save(s){ write(storeKey, s); }
  function cooldowns(){ return read(cooldownKey, {}); }
  function canLaunch(name){ const c=cooldowns(); return !c[name] || Date.now()-c[name] >= cooldownMs; }
  function ships(prefix){ return [0,1,2].map((_,i)=>({id:prefix+i,hp:100,max:100})); }
  function nearestRepublic(target){ let best=null, d=Infinity; galaxyPlanets.forEach(p=>{ if(p.control!=='republic')return; const n=Math.hypot(p.x-target.x,p.y-target.y); if(n<d){d=n;best=p;} }); return best || galaxyPlanets.find(p=>p.control==='republic'); }
  function update(s){
    const now=Date.now();
    if(s.status==='travel' && now>=s.arrivalAt){ s.status='battle'; s.battleAt=now; s.lastTick=now; s.log=['Флот прибыл к планете. Начинается орбитальное сражение.']; }
    if(s.status==='battle'){
      ...
      if(now-(s.battleAt||now) >= battleDurationMs) finish(s, s.winner || (Math.random()<.5?'republic':'cis'));
    }
    if(s.status==='finished'){
      const steps=Math.floor((now-(s.lastRegen||now))/regenMs);
      if(steps>0){ [...s.republic,...s.cis].forEach(x=>x.hp=Math.min(x.max,x.hp+x.max*.15*steps)); s.lastRegen=(s.lastRegen||now)+steps*regenMs; }
    }
    save(s); return s;
  }
  function damage(s){
    ...
    hit(ca,.35+Math.random()*1.55+(s.winner==='republic'?.45:0));
    hit(ra,.35+Math.random()*1.55+(s.winner==='cis'?.45:0));
  }
  function finish(s,winner){
    s.status='finished'; s.result=winner; s.finishedAt=Date.now(); s.lastRegen=Date.now();
    const winSide = winner==='republic' ? s.republic : s.cis;
    const loseSide = winner==='republic' ? s.cis : s.republic;
    winSide.forEach(x=>{ x.hp=Math.max(12, x.hp); });
    loseSide.forEach(x=>{ x.hp=Math.max(0, Math.min(x.hp, 18)); });
    if(winner==='republic'){ const r=controls(); r[s.target]='republic'; setControls(r); applyControls(); s.log=[`Планета ${s.target} захвачена Республикой.`]; }
    else s.log=[`Штурм системы ${s.target} провален. КНС удерживает планету.`];
    stopMusic();
  }
  function launch(target){
    if(!target || target.control!=='cis' || !canLaunch(target.name)) return;
    ...
    save({target:target.name,origin:origin.name,startedAt:now,arrivalAt:now+travelMs,status:'travel',lastTick:now,lastRegen:now,winner:Math.random()<.5?'republic':'cis',republic:ships('r'),cis:ships('c'),log:['Флот выслан.']});
    renderGalaxy(); galaxyFocusPlanet(target); startTimer();
  }
  function fmt(ms){ ms=Math.max(0,ms); const h=Math.floor(ms/3600000), m=Math.floor(ms%3600000/60000); return `${String(h).padStart(2,'0')}ч ${String(m).padStart(2,'0')}м`; }
  const oldPanel = renderGalaxyPanel;
  renderGalaxyPanel = function(planet){
    oldPanel(planet); if(!galaxyPanel||!planet)return;
    const s=state(); const active=s&&s.target===planet.name&&s.status!=='finished';
    const box=document.createElement('div'); box.className='galaxy-war-panel';
    if(planet.control==='cis'){
      const disabled=!canLaunch(planet.name)||active;
      if(active) box.innerHTML = `<p class="eyebrow">Флотская операция</p>${statusHtml(s)}`;
      else box.innerHTML = `<p class="eyebrow">Флотская операция</p><p>Система удерживается КНС. Можно направить ударную группу Venator для захвата и организации боевого вылета.</p><button class="galaxy-fleet-button galaxy-launch-btn" ${disabled?'disabled':''}>${disabled?'Захват доступен раз в день':'Выслать флот для захвата'}</button>`;
      box.querySelector('button')?.addEventListener('click',()=>launch(planet));
    } else box.innerHTML = `<p class="eyebrow">Флотская операция</p><p>Захват доступен только для вражеских планет КНС.</p>`;
  ...
  function statusHtml(s){
    if(s.status==='travel'){ const pct=Math.min(100,(Date.now()-s.startedAt)/(s.arrivalAt-s.startedAt)*100); return `<p>Флот в пути из системы <b>${s.origin}</b>.</p><div class="galaxy-war-progress"><i style="width:${pct}%"></i></div><p class="galaxy-war-time">До прибытия: ${fmt(s.arrivalAt-Date.now())}</p>`; }
    if(s.status==='battle'){ const left=fmt((s.battleAt||Date.now())+battleDurationMs-Date.now()); return `<p>У орбиты идёт сражение. Видео прибытия воспроизводится при каждом входе в карту, музыка идёт циклом.</p><p class="galaxy-war-time">До завершения боя: ${left}</p>${hpHtml(s)}`; }
    return `${s.log?.[0]||'Бой завершён.'}${hpHtml(s)}`;
  }
  function hpHtml(s){ const row=(t,a,c)=>`<div class="galaxy-hp-row ${c}"><b>${t}</b>${a.map(x=>`<span><i style="width:${Math.round(x.hp)}%"></i><em>${Math.round(x.hp)}%</em></span>`).join('')}</div>`; return `<div class="galaxy-hp-grid">${row('Venator',s.republic,'rep')}${row('КНС',s.cis,'cis')}</div>`; }
  const oldRender = renderGalaxy;
  renderGalaxy = function(){ applyControls(); oldRender(); renderShips(); maybeVideo(); startFleetLoop(); };
  function img(ns,href,x,y,w,h,cls,rot){
  ...
  function renderShips(){
    ...
    const now=Date.now();
    galaxyPlanets.filter(p=>p.control==='cis').forEach(p=>{
      for(let i=0;i<3;i++){
        const a=(now/2600+i*2.094+p.x*.013+p.y*.007)%(Math.PI*2);
        const x=p.x+Math.cos(a)*30, y=p.y+Math.sin(a)*19;
        const pulse=1+Math.sin(now/700+i)*.04;
        layer.appendChild(img(ns,assets.cis,x,y,Math.round(25*pulse),Math.round(15*pulse),'galaxy-ship galaxy-enemy-cruiser orbiting',0));
      }
    });
    ...
    layer.insertBefore(route,layer.firstChild);
    if(s.status==='travel'){
      ...
      layer.insertBefore(progress,route.nextSibling);
      for(let i=0;i<3;i++){
    ...
    if(s.status==='battle'){
      ...
      const sway=Math.sin(now/900)*8;
      for(let i=0;i<3;i++) layer.appendChild(img(ns,assets.venator,target.x-76+sway,target.y-34+i*34,38,22,'galaxy-ship republic-ship battle',0));
      for(let i=0;i<3;i++) layer.appendChild(img(ns,assets.cis,target.x+76-sway,target.y-34+i*34,31,18,'galaxy-ship cis-ship battle',0));
      for(let i=0;i<12;i++){
        const l=document.createElementNS(ns,'line'); const blue=i%2===0; const y=target.y-55+(i%6)*20+Math.sin(now/240+i)*4;
        const phase=(now/18+i*31)%80;
        l.setAttribute('x1',target.x+(blue?-62+phase*.35:62-phase*.35));
        l.setAttribute('y1',y);
        l.setAttribute('x2',target.x+(blue?8+phase*.35:-8-phase*.35));
        l.setAttribute('y2',y+(Math.sin(now/180+i)*8));
        l.setAttribute('class',`galaxy-laser ${blue?'blue':'red'}`);
        layer.appendChild(l);
      }
    }
  }
  ...
  function playVideo(s){
    ...
    overlay.innerHTML=`<video class="galaxy-arrival-video" src="${assets.video}" autoplay playsinline preload="auto"></video><div class="galaxy-video-shade"></div><div class="galaxy-video-title">ПРИБЫТИЕ ФЛОТА РЕСПУБЛИКИ</div><button type="button" class="galaxy-video-play" hidden>Запустить запись</button>`;
    ...
    v.addEventListener('error',()=>{ playBtn.hidden=false; playBtn.textContent='Запустить бой'; },{once:true});
  ...
  function maybeVideo(){ const s=state(); if(!s||s.status!=='battle'||!galaxyView||galaxyView.hidden)return; const key=battleVideoKey(s); if(videoMark===key || videoMark===key+':done')return; videoMark=key; playVideo(s); }
  let fleetLoop = false;
  ...
  function startTimer(){ if(timer)return; timer=setInterval(()=>{ if(galaxyView&&!galaxyView.hidden){ const before=state(); renderGalaxyPanel(galaxySelected); renderGalaxyPlanetList(); if(before && before.status==='finished') renderGalaxy(); } },2500); startFleetLoop(); }
